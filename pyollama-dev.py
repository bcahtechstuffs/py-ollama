...
# finally, bcah learned how to use classes
# version 0.3dev (June-03-'25)
...
try:
    import ollama, os, re, time, subprocess, datetime
...
/stat   : Show statistics for previous response.
    ...
    def generate_chat_response(self, user_input):
        try:
            ...
            if not self.rawoutput:
                with Live(console=self.console, vertical_overflow="ellipsis", screen=False, refresh_per_second=20, transient=True) as lv:
                    rpanel = Panel.fit(
                        current,
                        border_style="blue"
                    )
    ...
    def start(self):
        """Starts the py-ollama application."""
        rich.print("[bold blue]py-[bold yellow]ollama")
        rich.print(Panel.fit("[bold yellow](i) This is dev build, look out for potential bugs!", border_style="bold yellow"))
        rich.print(Panel.fit("""[bold white][Version: 0.3dev - June 6 '25][/bold white]
