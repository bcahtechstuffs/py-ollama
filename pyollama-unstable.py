...
class pyollama: # one big chunk of code
    ...
    def read_file(self): # file reading (W.I.P)
        ...
        try:
            with open(self.fileLocation, 'r') as fl:
...
You may need to terminate all Ollama processes manually""", border_style="yellow"))
    ...
    def pull_model(self, model_name): # pull a model from le internet
...
You may need to get a response from LLM atleast once.""", border_style="bold red"))
        except Exception as e:
...
/file      : Add file for current prompt (can be only used for one prompt) (W.I.P)''', border_style="green"))
...
[bold yellow]This is a development build, be REALLY wary of bugs n' stuffs![/bold yellow]
...
[bold white]If you see this message while using a desktop enviroment (or perhaps using macOS), please create new issue to pyollama's Github repository""", border_style="yellow"))
        if (len(sys.argv)) > 1:
            ...
            while True:
                ...
                if model_input[0] == '/':
                    ...
                    # rich.print(f"Selected model: [bold white]{model_input}") this line will removed due to duplicate line
                    if self.switch_model(model_input):
                        rich.print("[white italic]Type [blue italic]/help[/blue italic] for list of commands.")
                        break
                    else:
